...
<div class="container-fluid">
    <h1 class="h3 mb-4 text-gray-800">Edit Kelas: {{ $kela->nama_kelas }}</h1>
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('manajemen.kelas.update', $kela) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="walikelas_id" name="walikelas_id">
                        ...
                        @foreach($gurus as $guru)
                            <option value="{{ $guru->id }}" @selected(old('walikelas_id', $kela->walikelas_id) == $guru->id)>{{ $guru->name }}</option>
                        @endforeach
                    </select>
                </div>
